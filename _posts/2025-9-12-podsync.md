...
在VPS上用 Docker 部署 yangtfu/podsync，把 Bilibili 视频自动转成可订阅的播客（Podcast）源，并通过自己的域名提供 RSS 订阅。
...
- 如果未安装，可参考官方教程(https://docs.docker.com/engine/install/ubuntu/)，或使用 apt 版 compose plugin。

###二、项目说明与仓库信息
- 使用版本：yangtfu/podsync（默认分支为 bilibili，Go 语言，MIT License）
- 仓库主页：https://github.com/yangtfu/podsync
- 该 fork 在原 mxpv/podsync 的基础上，增加了 Bilibili 的抓取能力。本文将基于这个版本构建 Docker 镜像和运行服务。
